...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            # Suggesting C-T columns if they exist as a default range
            ...
            use_normalization = st.sidebar.checkbox("Apply Normalization (Center Scaling)", value=False, help="Subtracts mean and divides by standard deviation for each compound.")
            ...
            with col1:
                ...
                try:
                    ...
                    st.download_button(
                        ...
                        data=svg_bytes,
                        file_name="heatmap_vector.svg",
                        mime="image/svg+xml",
                    )
                except Exception as e:
                    st.error("SVG export requires 'kaleido' package.")
            ...
            with col3:
                ...
                try:
                    png_bytes = fig.to_image(format="png", width=1200, height=max(800, len(plot_df)*20), scale=2)
                    st.download_button(
                        label="Download as PNG (High-Res)",
                        data=png_bytes,
                        file_name="heatmap_highres.png",
                        mime="image/png",
                    )
                except Exception as e:
            ...
            st.download_button("Download CSV Data", csv, "processed_data.csv", "text/csv")
